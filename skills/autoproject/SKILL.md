...
8. **Steering.** Anything the user says about the active run ("try X next", "avoid Y") goes into the scratch file's `## User guidance` for the next maker. Changes to limits or the rubric are written to the scratch file and apply from the next step. "Stop" ends the run after the current step, and no new thread starts.

...
2. At any point, treat “go,” “let’s just go,” “run it,” or “use what you have” as authorization to infer and default every unresolved input, create the run files, and start the ratchet immediately without further questions or approvals.
...
7. Continue with the Orchestrator: ask separately for each still-missing value in this order — `MAX_ITERATIONS`, `MAX_CONSECUTIVE_FAILURES`, then `SUCCESS_CONDITION` — and show the resolved three values for approval.
8. Present the approved configuration with the Final summary Template and ask whether to run; revise the relevant phase when feedback changes an approved input.
9. On run approval, preflight the repository and baseline, create the run directory, write the three approved run files, and read `orchestrator.md` to execute the ratchet exactly as instructed.
...
- Ask one missing input per turn with 2–4 numbered, concrete choices and mark one recommendation; accept a free-form answer even when it is not one of the choices.
...
- For each draft or Orchestrator-values approval, print the artifact or values followed by exactly:
...
| `MAX_ITERATIONS` | Recommend `5`, but ask unless supplied or the run-now override applies. |
| `MAX_CONSECUTIVE_FAILURES` | Recommend `3`, but ask unless supplied or the run-now override applies. |
| `SUCCESS_CONDITION` | Recommend `none`, but ask whether an observable early-stop condition exists unless supplied or the run-now override applies. |

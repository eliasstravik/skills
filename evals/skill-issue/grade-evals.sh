...
grade_arm() {
  local arm_dir="$1"
  local eval_dir="$(dirname "$(dirname "$arm_dir")")"
  local eval_base="$(basename "$eval_dir")"
  local eval_id="${eval_base#eval-}"
  local assertion_ids
...

  {
    ...
    echo
    echo "Use only staged instructions and evidence inside $ITERATION_DIR. Do not invoke another model, skill, helper agent, web search, browser, service, credential, or external temporary directory."
    ...
    echo "Read the arm prompt, transcript, complete events, objective-evidence.json, every output, and workspace state under $arm_dir."
    ...
    echo "Apply the burden of proof to PASS and return only JSON matching the schema."
  ...
  jq --argjson grader_duration "$grader_duration" '. + {grader_duration_seconds: $grader_duration, total_duration_seconds: (.executor_duration_seconds + $grader_duration)}' "$arm_dir/timing.json" > "$tmp_file"
  ...
  jq --argjson command_calls "$command_calls" --argjson file_change_calls "$file_change_calls" --argjson errors "$errors" --argjson output_chars "$output_chars" --argjson transcript_chars "$transcript_chars" '. + {execution_metrics: {tool_calls: {command_execution: $command_calls, file_change: $file_change_calls}, total_tool_calls: ($command_calls + $file_change_calls), total_steps: ($command_calls + $file_change_calls), errors_encountered: $errors, output_chars: $output_chars, transcript_chars: $transcript_chars}}' "$arm_dir/grading.json" > "$tmp_file"
...
if [[ "$failures" -ne 0 ]]; then echo "$failures grader arm(s) failed" >&2; exit 1; fi

for eval_dir in "$ITERATION_DIR"/eval-*; do
  jq --slurpfile grade "$eval_dir/with_skill/run-1/grading.json" '.assertions = ($grade[0].expectations | map(.text))' "$eval_dir/eval_metadata.json" > "$eval_dir/eval_metadata.with-assertions.json"

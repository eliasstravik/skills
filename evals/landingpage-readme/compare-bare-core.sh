...
stage_side() {
  ...
  mkdir -p "$destination"
  find "$source_arm/outputs" -maxdepth 1 -type f ! -name response.md -exec cp {} "$destination/" \;
  if [[ "$eval_id" == "4" ]]; then
    sed \
      -e 's#/with_skill/run-1/#/[run]/#g' \
      -e 's#/without_skill/run-1/#/[run]/#g' \
      "$source_arm/transcript.md" > "$destination/response.md"
  fi
}
...
run_comparison() {
...

  {
    ...
    echo
    echo "Use only the staged comparator instructions and blinded inputs inside $ITERATION_DIR. Do not inspect arm directories, benchmark grades, skill files, mappings, transcripts outside the blinded inputs, another model, helper agent, web search, browser, service, credential, or external temporary directory."
...
for eval_id in 1 3; do
  eval_dir="$(find "$ITERATION_DIR" -mindepth 1 -maxdepth 1 -type d -name "eval-${eval_id}-*" -print -quit)"
  stage_side "$eval_dir/without_skill/run-1" "$INPUT_ROOT/eval-$eval_id/A" "$eval_id"
  stage_side "$eval_dir/with_skill/run-1" "$INPUT_ROOT/eval-$eval_id/B" "$eval_id"
done

for eval_id in 2 4; do
  eval_dir="$(find "$ITERATION_DIR" -mindepth 1 -maxdepth 1 -type d -name "eval-${eval_id}-*" -print -quit)"
  stage_side "$eval_dir/with_skill/run-1" "$INPUT_ROOT/eval-$eval_id/A" "$eval_id"
  stage_side "$eval_dir/without_skill/run-1" "$INPUT_ROOT/eval-$eval_id/B" "$eval_id"
done

pids=()
for eval_id in 1 2 3 4; do

...
test -s "$ITERATION_DIR/bare-core-SKILL.md"
test "$(awk 'BEGIN { fences=0; body=0 } /^---$/ { fences++; next } fences >= 2 { body++ } END { print body }' "$ITERATION_DIR/bare-core-SKILL.md")" -le 20
test "$(rg -c '^# Landingpage README$' "$ITERATION_DIR/bare-core-SKILL.md")" -eq 1
test "$(rg -c '^## Recipe$' "$ITERATION_DIR/bare-core-SKILL.md")" -eq 1
test "$(rg '^## Details$' "$ITERATION_DIR/bare-core-SKILL.md" | wc -l | tr -d ' ')" -eq 0
